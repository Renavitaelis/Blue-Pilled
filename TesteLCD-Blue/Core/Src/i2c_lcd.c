...
void lcd_clear_all(I2C_LCD_HandleTypeDef *lcd)
{
    ...
    for (int character = 0; character < 80; character++)
        lcd_send_cmd(lcd, ' ');  // Write a space on each position
}

void lcd_clear_row(I2C_LCD_HandleTypeDef *lcd, int row)
{
    lcd_gotoxy(lcd, 0, row); // Move cursor to the beginning of selected row
...
{
    lcd_send_cmd(lcd, ch);  // Send a character to the display
}
...
{
    while (*str) lcd_send_cmd(lcd, *str++);  // Send each character in the string
}
...
{
	lcd_send_cmd(lcd, 0x1C); //Shifts one column to the left
}
...
{
	lcd_send_cmd(lcd, 0x18); //Shifts one column to the right
}
